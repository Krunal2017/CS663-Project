% im=imread('../src/1.jpg');
im=imread('../SampleImages/Butterfly/10.png');
[m n p]=size(im);
res_im=im(3:m-2,3:n-2,1);
window_size=5;
...
channel1=fmat(:,:,1);
xbar=mean(channel1,2);
mean_deducted=channel1-xbar;
...
normalized_V = normalized_V(:, indices);
tx=normalized_V(:,1:3);
clusters=5;
idx=kmeans(tx,clusters);
